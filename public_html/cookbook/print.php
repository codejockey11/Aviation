...
if ($selection)
{
	...
	FormatRecipe($title, $author, $ingrediants, $steps, 0);
}
else
{
	$d = new Database();
	...
	if ($d->GetRowCount() > 0)
	{
		...
		while($row = $d->FetchRow())
		{
			...
			if ($prevTitle != $title)
			{
				FormatRecipe($prevTitle, $prevAuthor, $ingrediants, $prevSteps, $page);
...
function FormatRecipe($title, $author, $ingrediants, $steps, $page)
{
